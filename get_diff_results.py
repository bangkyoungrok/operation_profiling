...
def get_diff_norm(x, y):
    ...
    return diff_mean.item(), diff_mean.div_(base_mean).item(), diff_norm_L1.item(), diff_norm_L2.item(), diff_norm_L1.div_(base_norm_L1).item(), diff_norm_L2.div_(base_norm_L2).item()


if __name__ == '__main__':

    device_list = ['gpu', 'tpu']
    op_list = ['softmax', 'GeLU', 'tanh', 'sigmoid', 'swish']
    op_result = {}
    ...
    for op_name, op_val in op_result.items(): # 이 밑에는 device별로 있음
        ...
        for device, op_result_tensor in op_val.items():
            diff_mean, base_ratio, diff_norm_L1, diff_norm_L2, diff_ratio_L1, diff_ratio_L2 = get_diff_norm(op_result_tensor, op_val['gpu'])
            item = {
                'op': op_name,
                'device': device,
                'diff_mean' : diff_mean,
