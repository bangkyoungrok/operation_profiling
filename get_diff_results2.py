...
if __name__=='__main__':
    ...
    for op in op_list:
        ...
        for device in device_list:
            with open(f"./tensors/{device}_output/10/{device}_torch_{op}_output.pickle", "rb") as fr:
    ...
    result_pd.to_csv('./result/10/result.csv')
    df_ratio_mean.to_csv('./result/10/result_ratio_mean.csv')
    df_ratio_top5.to_csv('./result/10/result_ratio_top5.csv')
    df_ratio_bottom5.to_csv('./result/10/result_ratio_bottom5.csv')
    df_prob.to_csv('./result/10/result_prob.csv')
